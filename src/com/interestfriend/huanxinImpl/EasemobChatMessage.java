...
public class EasemobChatMessage {
	...
	public static void main(String[] args) {
		 ...
		 * queryStrNode.put("limit", "20"); ObjectNode messages =
		 * getChatMessages(queryStrNode);
		 */
		...
		/*
		 * // 聊天消息 分页获取 ObjectNode queryStrNode2 = factory.objectNode();
		 * queryStrNode2.put("limit", "20"); // 第一页 ObjectNode messages2 =
		 * getChatMessages(queryStrNode2); // 第二页 String cursor =
		 * messages2.get("cursor").asText(); queryStrNode2.put("cursor",
		 * cursor); ObjectNode messages3 = getChatMessages(queryStrNode2);
		 */
	}
	...
	public static ObjectNode getChatMessages(ObjectNode queryStrNode) {
		...
		try {
			...
			if (null != queryStrNode && queryStrNode.get("ql") != null
				...
				rest = "ql="
						+ java.net.URLEncoder.encode(queryStrNode.get("ql")
								.asText(), "utf-8");
			}
			...
			URL chatMessagesUrl = HTTPClientUtils.getURL(EasemobConstants.APPKEY
					.replace("#", "/") + "/chatmessages?" + rest);

			objectNode = HTTPClientUtils.sendHTTPRequest(chatMessagesUrl,
